...
# FIXED: use script-relative base directory + CLI paths instead of hardcoded absolute/relative paths
...
def load_anns(path: str, filename: str) -> Dict[str, Any]:
    # Loads an annotation file
    with open(path + filename, "r", encoding="utf-8") as f:
        anns = json.load(f)
    return anns

def save_dataset(dataset: Dict[str, Any], filename: str, annotations_dir: str) -> None:
    # Saves an annotation file
    path = Path(annotations_dir)
    path.mkdir(parents=True, exist_ok=True)
    with open(path / filename, 'w', encoding='utf-8') as f:
        json.dump(dataset, f, indent=0)
    logger.info("Saved dataset to disk as %s!", filename)
...
def filter_classes(dataset: Dict[str, Any], strict: bool = False) -> Dict[str, Any]:
    """
    ...
    validate_or_warn(len(keep) == len(class_names), "Expected {} kept categories, got {}.".format(len(class_names), len(keep)), strict)
    ...
    logger.info("Removed %s / %s images by category.", len(dataset['images']) - len(img_id_table.keys()), len(dataset['images']))

    imgs_out = []
    imgs_ids_out = []
    anns_out = []

    for img_id in tqdm(img_id_table, desc="Collect annotations", disable=not logger.isEnabledFor(logging.INFO)):
        imgs_ids_out.append(int(img_id))
        anns_out += img_id_table[img_id]

    for img_id in tqdm(imgs_ids_out, desc="Collect images", disable=not logger.isEnabledFor(logging.INFO)):
    ...
    # Check output
    validate_or_warn(
        len(dataset['images']) == len(set(ann['image_id'] for ann in dataset['annotations'])),
        "Image/annotation image_id mismatch after filter_classes.",
        strict
    )

    return dataset

def copy_image_files(img_ids: List[Any], foldername: str, images_dir: str, strict: bool = False) -> None:
    """
    ...
    for img in tqdm(img_ids, desc="Copy images", disable=not logger.isEnabledFor(logging.INFO)):
        ...
        try:
            ...
            logger.error("Folder %s does not exist. Please create it and try again.", path / foldername)
            return
    validate_or_warn(count == len(img_ids), "Copied {} images but expected {}.".format(count, len(img_ids)), strict)

    logger.info("Copied %s images to %s", count, path / foldername)

def make_base_dataset(anns_train1: Dict[str, Any], anns_train2: Dict[str, Any], anns_val: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    logger.info("Found %s annotations in total.", len(anns_in))
    anns_count = {10:0, 92:0, 93:0, 94:0}
    seen = []

    for ann in tqdm(anns_in, desc="Filter base annotations", disable=not logger.isEnabledFor(logging.INFO)):
        if (int(ann['category_id']) in target_classes) and (ann['id'] not in seen):
            anns_out.append(ann)
            seen.append(ann['id'])
            anns_count[int(ann['category_id'])] += 1
    logger.info("Filtered %s annotations containing traffic lights.", len(anns_out))

    images_out =[]
    images_in = anns_train1['images'] + anns_train2['images'] + anns_val['images']
    images_keep = [int(x['image_id']) for x in anns_out]
    logger.info("Found %s images in total. Keeping %s images.", len(images_in), len(images_keep))
    
    for image in tqdm(images_in, desc="Filter base images", disable=not logger.isEnabledFor(logging.INFO)):
        if int(image['id']) in images_keep:
            images_out.append(image)

    logger.info("Added %s images containing traffic lights.", len(images_out))
...
def make_coco_refined(dataset_in: Dict[str, Any], dataset_relabelled: Dict[str, Any], strict: bool = False) -> Dict[str, Any]:
    """
    ...
    logger.info("Found %s / %s relabelled annotations.", count, count_total + count)
    validate_or_warn(len(anns_out) == len(dataset_in['annotations']), "Annotation count changed in make_coco_refined.", strict)
    validate_or_warn(count+count_total == len(dataset_in['annotations']), "Relabelled+unchanged counts do not match input in make_coco_refined.", strict)
    validate_or_warn(len(dataset['images']) == len(dataset_in['images']), "Image count changed in make_coco_refined.", strict)
    validate_or_warn(len(dataset_in['annotations']) == len(dataset['annotations']), "Output annotation count mismatch in make_coco_refined.", strict)
    
    return dataset

def make_coco_traffic(
    dataset_train: Dict[str, Any],
    dataset_val: Dict[str, Any],
    dataset_add: Dict[str, Any],
    strict: bool = False,
    ...
    logger.info("Found %s images which will be split into train and val.", len(images_add))
    ...
    validate_or_warn(len(imgs_train) == num_train, "Train split size mismatch in make_coco_traffic.", strict)
    validate_or_warn(len(imgs_val) == num_val, "Val split size mismatch in make_coco_traffic.", strict)
    validate_or_warn(len(images_add) == (num_train + num_val), "Split totals do not match source image count in make_coco_traffic.", strict)
    ...
    for img_id in tqdm(imgs_train, desc="Append train split", disable=not logger.isEnabledFor(logging.INFO)):    
        images_train_out.append(images_table[img_id])
        anns_train_out += anns_table[img_id]

    for img_id in tqdm(imgs_val, desc="Append val split", disable=not logger.isEnabledFor(logging.INFO)):
    ...
    validate_or_warn(len(dataset_train["images"]) == (num_imgs_train_in + num_train), "Final train image count mismatch in make_coco_traffic.", strict)
    validate_or_warn(len(dataset_val["images"]) == (num_imgs_val_in + num_val), "Final val image count mismatch in make_coco_traffic.", strict)

    return dataset_train, dataset_val, imgs_train, imgs_val
    
def make_coco_traffic_extended(dataset_in: Dict[str, Any], dataset_append: Dict[str, Any], strict: bool = False) -> Dict[str, Any]:
    """
    ...
    validate_or_warn(len(dataset_out['licenses']) == 9, "Expected 9 licenses in extended dataset, got {}.".format(len(dataset_out['licenses'])), strict)
    ...
    validate_or_warn(
        len(dataset_out['images']) == (len(dataset_in['images']) + len(dataset_append['images'])),
        "Extended image count mismatch.",
        strict
    )
    validate_or_warn(
        len(dataset_out['annotations']) == (len(dataset_in['annotations']) + len(dataset_append['annotations'])),
        "Extended annotation count mismatch.",
        strict
    )

    return dataset_out

def make_new_images(dataset: Dict[str, Any], imgs_train: List[Any], imgs_val: List[Any]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    """
    ...
    # FIXED: preserve all annotations per image_id (old dict-comprehension dropped all but one)
    table_anns = defaultdict(list)
    for ann in tqdm(dataset['annotations'], desc="Index annotations", disable=not logger.isEnabledFor(logging.INFO)):
    ...
    dataset_train['annotations'] = []
    for image_id in tqdm(imgs_train, desc="Build train annotations", disable=not logger.isEnabledFor(logging.INFO)):
        dataset_train['annotations'].extend(table_anns[image_id])
    ...
    dataset_val['annotations'] = []
    for image_id in tqdm(imgs_val, desc="Build val annotations", disable=not logger.isEnabledFor(logging.INFO)):
        dataset_val['annotations'].extend(table_anns[image_id])
...
def print_stats(dataset: Dict[str, Any]) -> None:
    ...
    logger.info("Number of images: %s\nNumber of annotations: %s\n", num_images, num_anns_total)
...
if __name__ == "__main__":
    ...
    # FIXED: replace bare print with structured logging and runtime verbosity control
    logging.basicConfig(level=logging.INFO if args.verbose else logging.WARNING, format="%(levelname)s:%(name)s:%(message)s")

    # 0. Dataset: COCO Traffic Lights
    path = str(Path(args.annotations) / "21_coco_sub_all_traffic") + "/"
    ...
    logger.warning("----------\nDataset 1")
    path = str(Path(args.annotations)) + "/"
    ...
    anns_val = load_anns(path, file_val)
    dataset2train = make_coco_refined(anns_train, anns_relabelled, strict=args.strict)
    dataset2val = make_coco_refined(anns_val, anns_relabelled, strict=args.strict)
    ...
    anns_train = load_anns(path, file_train)
    anns_val = load_anns(path, file_val)
    path_add = str(Path(args.annotations) / "21_coco_sub_all_traffic") + "/"
    file_val_add = "instances_val2017Relabelled.json"
    anns_add = load_anns(path_add, file_val_add)
    logger.info("%s", len(anns_add['annotations']))
    anns_add = filter_classes(anns_add, strict=args.strict)
    logger.info("%s", len(anns_add['annotations']))
    print_stats(anns_train)
    print_stats(anns_val)
    print_stats(anns_add)
    train_out, val_out, imgs_train, imgs_val = make_coco_traffic(anns_train, anns_val, anns_add, strict=args.strict)
    ...
    #copy_image_files(imgs_train, "trainTraffic", args.input)
    #copy_image_files(imgs_val, "valTraffic", args.input)

    # 3. Dataset: COCO Traffic Extended
    logger.warning("----------\nDataset 3")
    path = str(Path(args.annotations) / "30_lisa_sub") + "/"
    ...
    dataset_train = make_coco_traffic_extended(train_out, train_append, strict=args.strict)
    dataset_val = make_coco_traffic_extended(val_out, val_append, strict=args.strict)
